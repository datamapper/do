...
def download_file(directory, file)
  when_writing "downloading #{file}" do
    cd(directory) do
      system("wget #{file} || curl -C - -O #{file}")
    end
  end
end
...
namespace :build do
  ...
  namespace :win32 do
    ...
    # download ruby
    file "#{STASH}/ruby-1.8.6-p287.tar.gz" => STASH do
      download_file(STASH, "ftp://ftp.ruby-lang.org/pub/ruby/1.8/ruby-1.8.6-p287.tar.gz")
    end

    # extract ruby
    file "#{CROSS}/ruby-1.8.6-p287" => [CROSS, "#{STASH}/ruby-1.8.6-p287.tar.gz"] do
    ...
    # compile ruby
    file "#{CROSS}/bin/ruby.exe" => "#{CROSS}/ruby-1.8.6-p287/Makefile" do
      when_writing "cross-compiling ruby" do
        cd "#{CROSS}/ruby-1.8.6-p287" do
          sh "make"
          sh "make install"
        end
      end
    ...
    file "#{CROSS}/ruby-1.8.6-p287/Makefile" => ["#{CROSS}/ruby-1.8.6-p287/Makefile.in.bak"] do
    ...
    file "#{CROSS}/ruby-1.8.6-p287/Makefile.in.bak" => "#{CROSS}/ruby-1.8.6-p287" do
    ...
    task :correct_makefile_in do
      when_writing "correcting Makefile.in" do
        makefile_in = "#{CROSS}/ruby-1.8.6-p287/Makefile.in"
      end
    end
  end
end
